...
class OtherFeatureExtractor(nn.Module):
    def __init__(self, n_cnn_layers=3, n_rnn_layers=5, rnn_dim=512, n_class=29, n_feats=128, stride=2, dropout=0.1):
        ...
        self.lstm = nn.LSTM(input_size=rnn_dim, hidden_size=512, num_layers=5,
                            batch_first=True)
        self.classifier = nn.Linear(in_features=512, out_features=28)
    ...
    def forward(self, x):
        ...
        h0 = torch.zeros(5, x.shape[0], self.rnn_dim).cuda()
        c0 = torch.zeros(5, x.shape[0], self.rnn_dim).cuda()
        x, _ = self.lstm(x, (h0, c0))
        x = self.classifier(x)
        return x
...
class UnidirectionalModel(nn.Module):
    ...
    def __init__(self, n_cnn_layers=3, n_rnn_layers=5, rnn_dim=512, n_class=29, n_feats=128, stride=2, dropout=0.1):
        ...
        self.classifier = nn.Sequential(
            nn.Linear(512, 512),  # birnn returns rnn_dim*2
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(512, n_class)
        )
...
class OtherModelDeeper(nn.Module):
    """Speech Recognition Model Inspired by DeepSpeech 2"""
    def __init__(self, n_cnn_layers=6, n_rnn_layers=5, rnn_dim=512, n_class=29, n_feats=128, stride=2, dropout=0.1):
        super(OtherModelDeeper, self).__init__()
        n_feats = n_feats//2
        self.cnn = nn.Conv2d(1, 32, 3, stride=stride, padding=3//2)  # cnn for extracting heirachal features
...
class OtherModelDeeperGRU(nn.Module):
    """Speech Recognition Model Inspired by DeepSpeech 2"""
    def __init__(self, n_cnn_layers=3, n_rnn_layers=7, rnn_dim=512, n_class=29, n_feats=128, stride=2, dropout=0.1):
        super(OtherModelDeeperGRU, self).__init__()
        n_feats = n_feats//2
        self.cnn = nn.Conv2d(1, 32, 3, stride=stride, padding=3//2)  # cnn for extracting heirachal features
